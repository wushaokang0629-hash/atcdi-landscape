...
const statements=projects.map(p=>escapeHtml(p.tagline));
const heroTitles=projects.map(p=>escapeHtml(p.heroTitle).replace(/\n/g,'<br>'));
...
function showProject(index,trigger){const p=projects[index];$('#detail-label').textContent=p.category;$('#detail-title').textContent=p.title;$('#detail-caption').textContent=p.caption;$('#detail-visual').innerHTML=`<img src="assets/${p.image}.jpg" alt="${escapeHtml(p.title+' · '+p.caption.split(' · ')[0])}">`;$('#detail-text').innerHTML='<p>'+escapeHtml(p.description).replace(/\n\n/g,'</p><p>')+'</p>'+ (p.gallery.length>1?'<section class="project-gallery" aria-label="项目图集"><h3>项目图集</h3>'+p.gallery.slice(1).map(g=>`<figure><img src="assets/${g.image}.jpg" loading="lazy" alt="${escapeHtml(p.title+' · '+g.caption)}"><figcaption>${escapeHtml(g.caption)}</figcaption></figure>`).join('')+'</section>':'')+'<p class="source">项目简介与图片来源：分院提供的项目资料及宣传画册。</p>';showPanel('#detail-panel',trigger);$('#detail-panel').scrollTop=0}
const info={
...
renderProjects();
$('#hero-title').innerHTML=heroTitles[0];$('#hero-project').innerHTML=statements[0];$('#hero-category').textContent=projects[0].category;
